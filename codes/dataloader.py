...
class load_manual(Dataset):
    ...
    def __init__(self, path_dir):
        ...
        self.event_names = get_file_name(self.path_dir, ".npy")  # 返回所有文件的文件名，带后缀
    ...
    def __getitem__(self, index):
        ...
        """
        neg_shape: torch.Size([30, 1, 260, 346])
        event_input_shape: torch.Size([30, 2, 260, 346])
        """
...
class load_auto(Dataset):
    ...
    def __getitem__(self, index):
        ...
        diff_t = torch.FloatTensor(
            np.array(event_data["index_t"] - event_data["ref_t"])
        )
        fx = event_data["fx"]
        ...
        event_input = torch.FloatTensor(
            np.concatenate((pos, neg), axis=1)
        )  # size = (step, channel, H, W)
